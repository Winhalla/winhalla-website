<script>
    ...
    function handleClose(userSettings) {
        isPopupOpened = false;
    }
            ...
            {#if !settingsAreOpened}
                <div>
                    ...
                    <div class="hidden  mt-8    md:flex justify-between">
                        <button class="/w-40 button button-brand    text-font">REJECT ALL</button>
                        <button class="/w-40 button button-brand    text-font"
                                on:click={() => handleSettingsButton()}>SETTINGS
                        </button>
                        <button class="/w-40 button button-brand" on:click={() => handleClose()}
                                style="background-color: #3de488">ACCEPT ALL
                        </button>
                    </div>
                </div>
                ...
                <div>
                        ...
                        {#each Object.entries(userSettings) as [key, value]}
                            <div class="w-full pl-2 py-1 text-2xl text-font flex justify-between items-center">
                                ...
                                <button class="hover:text-primary p-1    focus:outline-none"
                                        on:click={() => userSettings[key] = !value}>
                                    {#if value}
                                        <svg
                                ...
                                <div class="w-full">
                                    <button
                                        class="w-full flex justify-between items-center  ml-2 px-2  pr-4 md:pr-0  focus:outline-none"
                                        on:click={() => settings[key].opened = !settings[key].opened}>

                                        {key}

                                        {#if settings[key].opened}
                    ...
                    <div class="mt-8     flex flex-col items-center md:flex-row md:justify-between md:items-start">
                        <button class="w-45    button button-brand    text-font"
                                on:click={() => handleSettingsButton()}>CLOSE
                            SETTINGS
                        </button>
                        <button class="mt-3 md:mt-0  w-45    button button-brand" on:click={() => handleClose()}
                                style="background-color: #3de488">SAVE
                        </button>
                    </div>
                </div>
            {/if}
