<script>
    import { clickOutside } from "../../utils/clickOutside";

    export let data;

    let isDropdownOpen;
    const handleClick = () => {
        isDropdownOpen = !isDropdownOpen;
    };

    const viewedNotifications = () => {};

    const idToType = (id) => {
        if (id == 0) return "match finished";
        if (id == 1) return "quest finished";
    };
</script>

<style>
    .dropdown {
        top: 3.8rem;
        right: 0;
    }

    .bell-button:hover .bell {
        display: none;
    }
    .bell-button:hover .bell-hover {
        display: block;
    }

    .notification {
        border-radius: 10px;
        @apply flex justify-between px-5 py-3 mt-3 mb-1 relative overflow-hidden w-full;
    }
</style>

<div class="lg:relative">
    <div class="flex items-center h-full">
        <button
            class="focus:outline-none lg:ml-3 rounded bell-button"
            use:clickOutside
            on:click_outside={() => (isDropdownOpen = false)}
            on:click={() => handleClick()}>
            <div class="flex items-center relative">
                {#if isDropdownOpen}
                    <svg
                        class="w-5 h-6 fill-current"
                        viewBox="0 0 21 24"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="m20.333 17.16c-1.04-1.04-2.339-2.341-2.339-7.409 0-3.706-2.688-6.784-6.22-7.393l-.045-.006c.166-.238.265-.533.265-.851 0-.828-.672-1.5-1.5-1.5s-1.5.672-1.5 1.5c0 .318.099.613.268.856l-.003-.005c-3.578.614-6.266 3.692-6.266 7.399 0 5.068-1.296 6.367-2.339 7.409-.405.407-.655.968-.655 1.588 0 1.242 1.005 2.249 2.246 2.252h5.249c0 1.657 1.343 3 3 3s3-1.343 3-3h5.248c1.241-.004 2.246-1.011 2.246-2.252 0-.62-.25-1.181-.655-1.588zm-9.84 4.965c.207 0 .375.168.375.375s-.168.375-.375.375c-1.035-.001-1.874-.84-1.875-1.875h.75c.001.621.505 1.125 1.126 1.125z" /></svg>
                {:else}
                    <svg
                        class="w-5 fill-current bell"
                        viewBox="0 0 21 24"
                        xmlns="http://www.w3.org/2000/svg"><path
                            d="m19.945 15.512c-.8-.786-1.619-1.6-1.619-5.44-.005-3.881-2.832-7.101-6.539-7.717l-.046-.006c.165-.237.263-.531.263-.848 0-.828-.672-1.5-1.5-1.5s-1.5.672-1.5 1.5c0 .317.098.611.266.853l-.003-.005c-3.753.623-6.579 3.843-6.584 7.723v.001c0 3.84-.822 4.655-1.619 5.44-.653.577-1.062 1.417-1.062 2.352 0 1.732 1.404 3.135 3.135 3.135h.007 4.36c0 1.657 1.343 3 3 3s3-1.343 3-3h4.363.007c1.732 0 3.135-1.404 3.135-3.135 0-.935-.409-1.775-1.059-2.349l-.003-.003zm-9.441 6.613c-.621-.001-1.124-.504-1.125-1.125h2.251c-.001.621-.505 1.125-1.126 1.125zm7.36-3.376h-14.726c-.487-.003-.881-.398-.881-.886 0-.243.098-.463.256-.623 1.34-1.34 2.418-2.612 2.418-7.17 0-3.077 2.495-5.572 5.572-5.572s5.572 2.495 5.572 5.572c0 4.578 1.089 5.84 2.418 7.17.158.16.256.38.256.623 0 .488-.394.883-.881.886z" /></svg>
                    <svg
                        class="w-5 h-6 fill-current hidden bell-hover"
                        viewBox="0 0 21 24"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="m20.333 17.16c-1.04-1.04-2.339-2.341-2.339-7.409 0-3.706-2.688-6.784-6.22-7.393l-.045-.006c.166-.238.265-.533.265-.851 0-.828-.672-1.5-1.5-1.5s-1.5.672-1.5 1.5c0 .318.099.613.268.856l-.003-.005c-3.578.614-6.266 3.692-6.266 7.399 0 5.068-1.296 6.367-2.339 7.409-.405.407-.655.968-.655 1.588 0 1.242 1.005 2.249 2.246 2.252h5.249c0 1.657 1.343 3 3 3s3-1.343 3-3h5.248c1.241-.004 2.246-1.011 2.246-2.252 0-.62-.25-1.181-.655-1.588zm-9.84 4.965c.207 0 .375.168.375.375s-.168.375-.375.375c-1.035-.001-1.874-.84-1.875-1.875h.75c.001.621.505 1.125 1.126 1.125z" /></svg>
                {/if}
                <span
                    class="absolute top-0 right-0 w-2 h-2 rounded-full
                        bg-red-600" />
            </div>
        </button>
    </div>

    <div
        class:lg:hidden={!isDropdownOpen}
        class="pt-3 lg:pt-2 lg:py-2 lg:px-3 rounded-lg lg:bg-background
            lg:absolute lg:shadow-card dropdown z-50 w-92 border border-primary">
        <div>
            {#if data.notifications}
                <div>
                    <p class="ml-1">Notifications</p>
                    <div>
                        {#each data.notifications as notification}
                            <a
                                class="card notification flex items-center"
                                href={notification.id}>
                                <div class="progress-container">
                                    <p class="ml-2 mr-6 lg:mr-12 text-2xl">
                                        {notification.message}
                                    </p>
                                    <p
                                        class="ml-2 mr-6 lg:mr-12 text-light
                                            text-lg">
                                        {notification.tip}
                                    </p>
                                </div>
                                <span
                                    class="quest-goal text-sm text-font px-2
                                        py-1 bg-legendary rounded-lg">
                                    {idToType(notification.id)}
                                </span>
                            </a>
                        {/each}
                    </div>
                </div>
            {/if}
            {#if data.inGame}
                <div class="mt-5">
                    <p class="ml-1">Matchs in progress</p>
                    <div>
                        {#each data.inGame as match}
                            <a
                                class="card notification flex items-center"
                                href={match.id}>
                                <div class="progress-container">
                                    <p class="ml-2 mr-6 lg:mr-12 text-2xl">
                                        {match.type}
                                    </p>
                                    <p
                                        class="ml-2 mr-6 lg:mr-12 text-light
                                            text-lg">
                                        2:35:49
                                    </p>
                                </div>
                                <p class="quest-goal text-xl text-primary">
                                    {!match.isFinished ? match.progress : 'Waiting for others to finish'}
                                </p>
                            </a>
                        {/each}
                    </div>
                </div>
            {/if}
        </div>
    </div>
</div>
