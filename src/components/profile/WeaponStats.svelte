...
<div class="relative bg-variant rounded-xl  p-8 ml-18 xl:ml-10   h-auto" style="min-width: 21rem">
    ...
    <div class="ml-2 mt-1  bg-background p-6 pt-4  rounded-xl">
        ...
        <div class="mt-6 ml-2  text-xl  ">
            <p class="text-mid-light mt-">Time held: <b class="  font-normal text-primary text-2xl">{formatTime(selectedWeapon.matchtime)}</b>
            </p>
            <p class="text-mid-light mt-">Games played: <b
                    class="font-normal text-primary text-2xl">{selectedWeapon.games}</b>
            </p>

            <p class="text-mid-light mt-3">KOs: <b class="font-normal text-primary text-2xl">{selectedWeapon.kos}</b>
            </p>
            <p class="text-mid-light mt-">Damage dealt: <b class="font-normal text-primary text-2xl">{selectedWeapon.damagedealt}</b>
            </p>
        </div>
    </div>
        ...
        {#each data.sort(filterList[chosenFilter].filterFunction).slice(0, displayNumber) as weapon}
            <div class="w-full  mt-3  p-4 pl-6 bg-background rounded-xl">
                <button class="w-full flex items-center  relative"
                    ...
                    <div>
                        <img class="w-8" src="/assets/WeaponIcons/{weapon.name.replace('_', ' ')}.png" alt="">
                    </div>
                    <div class="mr-8  flex justify-between w-full">
                        <p class="ml-3">{weapon.name}</p>
                        <p class="text-green">
