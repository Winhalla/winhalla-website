...
<div class="pb-8 mt-9 px-4 lg:px-0">
    ...
    <div style="height:94vh;font-family: 'Roboto condensed', sans-serif; font-weight:700"
         class="lg:flex lg:justify-center  relative">
        <div style="color: rgba(253,253,253,0.80)" class="mt-26 lg:mt-48    ">
            <div class="overflow-hidden"
                 style=" height:{!isDocumentLoaded?'40%':`${document.getElementById('firstChildText').clientHeight+20}px`}">
                <div class:slider-container-transition={isTransitioningText} on:animationend={FlexSlider.changeOrder}
...
<!--    <div class="w-1/3"></div>-->
    <div class="overflow-hidden"
         style="height:{!isDocumentLoaded?'90%':`${document.getElementById('firstChildImg').clientHeight}px`}">
        <div class:slider-container-transition={isTransitioningImg} class="flex flex-nowrap flex-col">
            {#each items as item,i}
                <div class="slider-item" id="{i===0?'firstChildImg':''}">
                    <img src="/assets/screens/{item.file}" alt="screenshot"
                         class="mt-0 lg:mt-0 max-w-96" style="max-height: 25%">
                </div>
            {/each}
        </div>
    </div>
